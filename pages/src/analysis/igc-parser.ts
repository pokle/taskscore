...
/**
 * Get bounding box for a set of fixes
 */
export function getBoundingBox(fixes: IGCFix[]): {
  minLat: number;
  maxLat: number;
  minLon: number;
  maxLon: number;
  ...
  let minLat = fixes[0].latitude;
  let maxLat = fixes[0].latitude;
  let minLon = fixes[0].longitude;
  let maxLon = fixes[0].longitude;
